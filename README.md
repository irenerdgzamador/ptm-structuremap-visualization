...
1. Download ptm-structuremap-visualization.zip and extract (recommended location: `C:\`). 
2. Place the input file `quant_pgm_NM_LIMMA.tsv`(or an input table with the valid format) in `data/raw/`
3. Create and activate the Python 3.8 environment
4. Install dependencies: `pip install -r requirements.txt`
5. Run preprocessing:
   - Terminal: `python run_preprocessing.py`
   - or Notebook: `01_build_ptm_table.ipynb`
6. Run StructureMap: `02_run_structuremap.ipynb`
7. Generate plots: `03_plots.ipynb`
...
Ensure your local project folder contains this structure
...
This pipeline requires an input table generated by the **PTM-Analyzer** workflow.  
Specifically, the file expected as input is: `quant_pgm_NM_LIMMA.tsv`, that should be placed in the folder `data/raw/`

The input table must contain: 

	- "q" column:  Uniprot identifier

	- "a" column: residue letter

	- "n" column:  residue position

	- "pgm" column: containing peptide sequence, deltamass value and PTM position (deltamass is extracted from this column)



This table is the starting point of the entire workflow. Notebook 01 (build_ptm_table.ipynb) or the script `run_preprocessing.py` will read this file to build the binary PTM table.

**Domain annotation files (optional for Notebook 03)**

To generate domain‑annotated plots, provide one `.json` file per protein:

	- Download from UniProt → Family and domains section

	- Add any extra domain information if needed

	- Save as: *data/domains/domains_{UniProtID}.json*

**Protein list**

You must also provide `proteins.txt` file, with one UniProt ID per line.
...
Create and activate a virtual environment (CMD):
...
3. Click "Select Kernel" and select same interpreter
...
- Saves output in *data/processed/ptm_table/ptm_table.tsv*  
...
All outputs are saved in the folder defined in the notebook (modify the path as needed)
...
Run cell by cell.
Modify output paths depending on your project directory.
